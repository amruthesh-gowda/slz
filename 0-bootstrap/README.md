...
## This block is supposed to be run locally on machine exactly as explained below. 

1. Update the terraform.tfvars file
2. make sure backend.tf is commented 
3. run 
   1. $ gcloud auth application-default login
   2. Follow the instructions displayed, ( copy the link generated and use the link to login to your GCP account )
   3. After you login browser will return another string and use the string in your cli and paste. 
4. Once logged in, run terraform plan and apply
   1. out of all resources, look out for the Service-account and state bucket created. 
5. once resources are applied, 
6. uncomment the backend.tf and 
   1. update the bucket and prefix section with the state bucket metadata. 
7. Re run the terraform plan and apply
   1. This should ask to migrate the terraform state. 
   2. use the instruction to migrate the state to bucket. 